// *************************************
// BRR Sample Decoder in C++
//
// Author: Marc-Daniel DALEBA
// Date: 2025-10-27
// License: Public Domain CC0
// Description:
//   The BRR (Bit Rate Reduction) is an audio
//   compression method used on the SNES, The
...
/* This is the Table used by the S-DSP for the 
   4-Point Gaussian Interpolation Gaussian Filter */
...
int get_samples(char *filename);

int main(int argc, char **argv)
{
  std::vector<int16_t> brr;
  std::vector<int16_t> pcm;
  
  printf("BRR SAMPLE DECODER v1.0\n");
  printf("BY MARC-DANIEL DALEBA\n");
  
  if (get_samples(argv[1], brr) < 0) {
    printf("error: get_samples() failed.\n");
    return -1;
  }
...
int get_samples(char *filename, std::vector<int16_t> &out)
{
  ...
  while (!feof(f)) {
    // A BRR-Block is made out of 9 bytes.
    // The first byte is the BRR Header:
    //   SSSS.FFLE | S = Shift/Range
    //             | F = Filter
    //             | L = Loop Flag
    //             | E = End Flag
    //
    // The 8 following bytes store samples:
    //   AAAA.BBBB | A = Signed -8..7 Sample 1
    //             | B = Signed -8..7 Sample 2
    int header = getc(f);
    int shift  = (header >> 4) & 0xF;
    int filter = (header >> 2) & 0x2;
    for (int i = 0; i < 8; ++i) {
      int s  = getc(f);
      // First, we need signed-extended 16-bit samples
      int16_t sA = (s >> 4) & 0xF;
      if (sA >= 8)
        ;
